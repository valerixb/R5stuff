...
static int zynqmp_r5_a53_proc_irq_handler(int vect_id, void *data)
  ...
  prproc = rproc->priv;
  ipi_intr_status = (unsigned int)metal_io_read32(prproc->kick_io, IPI_ISR_OFFSET);
  if(ipi_intr_status & prproc->ipi_chn_mask)
    {
    atomic_flag_clear(&prproc->ipi_nokick);
    metal_io_write32(prproc->kick_io, IPI_ISR_OFFSET, prproc->ipi_chn_mask);
...
static struct remoteproc *zynqmp_r5_a53_proc_init(struct remoteproc *rproc,
  ...
  if(!rproc || !prproc || !ops)
    return NULL;
  ret = metal_device_open(prproc->kick_dev_bus_name, prproc->kick_dev_name, &kick_dev);
  if(ret)
    {
    LPRINTF("failed to open polling device: %d\n", ret);
    return NULL;
    }
  rproc->priv = prproc;
  ...
  metal_io_write32(prproc->kick_io, IPI_IER_OFFSET, prproc->ipi_chn_mask);
  rproc->ops = ops;
...
static void zynqmp_r5_a53_proc_remove(struct remoteproc *rproc)
  ...
  if(!rproc)
    return;
  prproc = rproc->priv;
  metal_io_write32(prproc->kick_io, IPI_IDR_OFFSET, prproc->ipi_chn_mask);
  dev = prproc->kick_dev;
  if(dev)
    {
...
static int zynqmp_r5_a53_proc_notify(struct remoteproc *rproc, uint32_t id)
  ...
  if(!rproc)
    return -1;
  prproc = rproc->priv;
  metal_io_write32(prproc->kick_io, IPI_TRIG_OFFSET, prproc->ipi_chn_mask);
  return 0;
  }
